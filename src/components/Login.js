...
const Login = (props) => {
  ...
  <div className="container  d-flex justify-content-center align-items-center" style={{height: "100vh"}}>
        <div className="card shadow-lg" style={{width: "24rem", backgroundColor:"#F5F5F5", border:""}}>
  
        <div className=" card-body">
        <h3 className="card-title text-center mt-3 mb-3"> Student Login</h3>
        {!showOTPInput&&( <> <div className="mb-3">
        <label htmlFor="otp" className="form-label fw-semibold">Registration No</label>
    ...
    <div className="mb-3">
    <label htmlFor="otp" className="form-label fw-semibold">Email Address</label>
    <input type="email" value={email} className="form-control" id="email" required placeholder="name@example.com" onChange={handleChange} />
  </div>
   <button disabled={email==="" || registrationNo===""} className="btn btn-dark" onClick={sendEmail}>Log In
        </button></>)}
  {showOTPInput&&(
  <>
  <div className="mb-3">
  <label htmlFor="otp" className="form-label fw-semibold">Enter OTP</label>
    <input type="text"
      ...
      <p className="mt-4 fw-semibold">Are you an Admin? <Link className="fw-bold " aria-current="page" to="/admin/login">  LogIn Here</Link> </p> 
        </div>
